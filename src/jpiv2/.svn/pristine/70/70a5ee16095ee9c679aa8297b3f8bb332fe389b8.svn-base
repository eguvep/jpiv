...
public class FlexFileChooser extends javax.swing.JFileChooser {
	...
	/**
	 * Set the type of file you want to select. Possible filetype ID's are: <br>
	 * DIR = directories <br>
	 * JVC = JPIV vector data (UTF-8 space delimited table, may or may not have
	 * a Tecplot compatible header) <br>
	 ...
	 *            Set whether multiple files can be selected or not.
	 */
	public void setFiletype(int ID, boolean multiSelection) {
		this.setMultiSelectionEnabled(multiSelection);
		this.setFileSelectionMode(FlexFileChooser.FILES_ONLY);
	 ...
	 *         identical with the first three characters of the filetype
	 *         description of the drop down list of filetypes in the dialog
	 *         (e.g. 'png').
	 */
	...
	private void addFileFilter(int ID) {
		switch (ID) {
		case DIR: {
			this.setFileSelectionMode(FlexFileChooser.DIRECTORIES_ONLY);
			break;
		}
		case JVC: {
	...
	private jpiv2.JpivFileFilter ffjvc = new jpiv2.JpivFileFilter(jvc,
			"jvc - JPIV vector data (UTF-8 space delimited table)");
	private jpiv2.JpivFileFilter ffjsc = new jpiv2.JpivFileFilter(jsc,
			"jsc - JPIV executable script (UTF-8 plain text)");
	private jpiv2.JpivFileFilter ffpng = new jpiv2.JpivFileFilter(png,
			"png - Portable Network Graphic (lossless compressed image)");
	private jpiv2.JpivFileFilter fftif = new jpiv2.JpivFileFilter(tif,
			"tiff - Tagged Image File Format (float data support)");
	private jpiv2.JpivFileFilter ffpgm = new jpiv2.JpivFileFilter(pgm,
			"pgm - Portable Grey Map (highly compatible format)");
	private jpiv2.JpivFileFilter ffimx = new jpiv2.JpivFileFilter(imx,
			"imx - DaVis compressed image format");
	private jpiv2.JpivFileFilter ffim7 = new jpiv2.JpivFileFilter(im7,
			"im7 - DaVis 7 compressed image format");
	private jpiv2.JpivFileFilter ffdat = new jpiv2.JpivFileFilter(dat,
			"dat - Tecplot vector data");
	private jpiv2.JpivFileFilter ffvec = new jpiv2.JpivFileFilter(vec,
			"vec - PIVware vector data");
	private jpiv2.JpivFileFilter ffnc = new jpiv2.JpivFileFilter(nc,
			"nc - netCDF PIV data (network common data format)");
	private jpiv2.JpivFileFilter ffeps = new jpiv2.JpivFileFilter(eps,
			"eps - encapsulated post script");
	private jpiv2.JpivFileFilter ffpdf = new jpiv2.JpivFileFilter(pdf,
			"pdf - portable document format");
	private jpiv2.JpivFileFilter ffemf = new jpiv2.JpivFileFilter(emf,
			"emf - enhanced meta format");
	private jpiv2.JpivFileFilter ffsvg = new jpiv2.JpivFileFilter(svg,
			"svg - scalable vector graphics");
	private jpiv2.JpivFileFilter ffswf = new jpiv2.JpivFileFilter(swf,
			"swf - shock wave format");
	private jpiv2.JpivFileFilter ffimgWrite = new jpiv2.JpivFileFilter(
			imgWrite, "png, tif, tiff, pgm - write compatible image formats");
	private jpiv2.JpivFileFilter ffimgRead = new jpiv2.JpivFileFilter(imgRead,
			"png, tif, tiff, pgm, imx, im7 - read compatible image formats");
	private jpiv2.JpivFileFilter ffvecRead = new jpiv2.JpivFileFilter(vecRead,
			"jvc, vec, dat, nc - read compatible vector formats");
	private jpiv2.JpivFileFilter ffgraWrite = new jpiv2.JpivFileFilter(
			graWrite,
			"eps, pdf, emf, svg, swf - write compatible vector graphics formats");
}